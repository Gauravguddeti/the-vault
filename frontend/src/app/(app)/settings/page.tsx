...
export default function SettingsPage() {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto w-full">
      ...
      <p className="text-neutral-400 mb-8">Manage your account preferences and assistant memory.</p>
      ...
      <div className="bg-surface border border-border/50 rounded-2xl p-6">
        ...
        {loading ? (
          ...
          <div className="text-center p-8 border border-dashed border-border/50 rounded-xl bg-background/50">
            <p className="text-neutral-400">The assistant hasn't learned any specific preferences yet.</p>
            <p className="text-sm text-neutral-500 mt-2">Chat more to build a personalized profile!</p>
          </div>
        ) : (
          <div className="space-y-3">
            <div>
              {memories.map((m) => (
                <div 
                  key={m.id}
                  className="flex items-center justify-between p-4 bg-background rounded-xl border border-border/50 group"
                >
                  <div>
                    <p className="text-sm font-medium">{m.content}</p>
                    <p className="text-xs text-neutral-500 mt-1 capitalize">{m.category} • {new Date(m.created_at).toLocaleDateString()}</p>
                  </div>
                  <button 
                    onClick={() => handleDelete(m.id)}
                    className="p-2 rounded-lg text-neutral-400 hover:text-red-400 hover:bg-red-400/10 transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100"
                    title="Delete Memory"
                  >
